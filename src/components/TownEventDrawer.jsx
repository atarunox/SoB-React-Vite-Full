...
export default function TownEventDrawer() {
  ...
  function drawEvent() {
    ...
    // Default type is 'instant' if not set
  ...
  return (
    <div className="p-4 bg-white rounded shadow-md space-y-4">
      ...
      <button onClick={drawEvent} className="btn btn-primary" disabled={deck.length === 0}>
        {deck.length === 0 ? 'No Cards Left' : 'Draw Event'}
      </button>
      <button onClick={reshuffleDeck} className="btn btn-secondary ml-2">
        Reshuffle Deck
      </button>
      ...
      {drawn && (
        <div className="border p-3 rounded bg-yellow-100">
          ...
          {drawn.flavorText && <p className="italic">{drawn.flavorText}</p>}
          <p><strong>Effect:</strong> {drawn.effect}</p>
          {drawn.type === 'lasting' && (
            <button onClick={activateLastingEvent} className="btn btn-success mt-2">
              Mark as Active
            </button>
          )}
        </div>
